...
const noIncomesMessage = document.getElementById('no-incomes-message');
...
function displayIncomes() {
    const listDiv = incomeListDiv;
    const data = incomes;
    const noMessage = noIncomesMessage;
    ...
    sortedData.forEach(entry => {
        ...
        entryItem.innerHTML = `
            <div class="entry-info">
                ...
                <p class="text-sm text-gray-600">Categoría: <span class="font-medium text-green-700">${entry.category}</span></p>
            </div>
            <div class="entry-actions">
                <p class="text-xl font-bold text-green-600">${entry.currency} ${entry.amount.toFixed(2)}</p>
                <button class="edit-btn bg-green-500 hover:bg-green-600 text-white p-2 rounded-full shadow-md" data-id="${entry.id}" data-type="incomes">
                ...
                <button class="delete-btn bg-gray-500 hover:bg-gray-600 text-white p-2 rounded-full shadow-md" data-id="${entry.id}" data-type="incomes">
...
function deleteEntry(event) {
    ...
    if (confirm('¿Estás seguro de que quieres eliminar este registro?')) {
        incomes = incomes.filter(entry => entry.id !== entryId);
        saveIncomes();
        displayIncomes();
...
function populateCategorySelects() {
    incomeCategorySelect.innerHTML = '<option value="">Selecciona una categoría</option>';
...
function openEditModal(event) {
    const entryId = parseInt(event.currentTarget.dataset.id);
    const entryToEdit = incomes.find(entry => entry.id === entryId);

    if (entryToEdit) {
        editIdInput.value = entryToEdit.id;
        editTypeInput.value = 'incomes';
...
editForm.addEventListener('submit', (event) => {
    ...
    const index = incomes.findIndex(entry => entry.id === id);

    if (index !== -1) {
        incomes[index] = { id, date, category, description, amount, currency };
        saveIncomes();
        displayIncomes();
        closeEditModal();
        showMessage('Ingreso actualizado exitosamente.', 'success');
    }
});
...
applyFilterBtn.addEventListener('click', displayIncomes); 
